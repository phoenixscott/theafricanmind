  ...
  return (
    <footer style={{ background: 'var(--black)', color: 'rgba(245,240,232,0.5)', padding: '3rem 2rem' }}>
      <div style={{
        ...
        <div>
          <p style={{
            ...
            fontSize: '1rem', fontWeight: 700, color: 'var(--cream)', marginBottom: '0.4rem'
          }}>
            The <span style={{ color: 'var(--gold)' }}>African</span> Mind
          </p>
          <p style={{ fontSize: '0.78rem', lineHeight: 1.5 }}>
            Rigorous thinking on Nigeria, Africa,<br />and the civilisational project of our century.
          </p>
        </div>
        <nav style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
          {['Nigeria', 'Africa', 'Solutions', 'Merit-Sovereignism', 'Fact Checks', 'About'].map(label => (
            <Link key={label} href={`/${label.toLowerCase().replace(' ', '-').replace('merit-sovereignism', 'merit-sovereignism')}`} style={{
      ...
      <div style={{
        ...
        fontFamily: "'Inter', system-ui, sans-serif",
        fontSize: '0.68rem', color: 'rgba(245,240,232,0.25)',
